...
import cz.ondramastik.bang.api
...
/**
  * Implementation of the BangService.
  */
class BangServiceImpl(
  ...
  /**
    ...
    */
  private def entityRef(id: String): EntityRef[PlayerManagerCommand] =
    clusterSharding.entityRefFor(PlayerManagerState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(GameManagerEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[GameManagerEvent]
  ): api.GreetingMessageChanged = {
    helloEvent.event match {
      case CardActionFinished(msg) =>
